...
class InferApp:
    ...
    def __init__(self, dataset_info, infer_device):
        ...
        # #be split by bbox quantity. 
            ...
            #Deprecated, SAM2 enables batched inference and so enables instance-wise generation in a straightforward manner.
        ...
        self.multimask_output_always = False  # By default, it will be true to match defaults from demo, this means that masks are always generated by propagating 
            ...
            for slice_idx in infer_slices[ax]:
                ...
                if avail_ps == []:
                    ...
                    if self.autoseg_infer:
                        ...
                        # propagation, we assume multi-mask to be used by default. It is highly unlikely for this strategy to be effective in any capacity.
    ...
    def binary_slice_predict(
        ...
        if mask_input is not None and mask_input.shape[0] > 1: #In this case the mask has been pre-initialised with a set of bboxes to obtain instance-level masks.
            ...
            if concat_points[0].shape[0] != mask_input.shape[0]:
                ...
                mask_input = mask_input.repeat(concat_points[0],1,1,1)
