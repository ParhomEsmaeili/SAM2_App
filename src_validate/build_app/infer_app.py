...
class InferApp:
    ...
    def __init__(self, infer_device, algorithm_state, enable_adaptation, algo_cache_name):
        ...
        self.glob_norm_bool = True #This is a variable denoting whether the global normalisation will be performed on the input image, i.e. whether the mean and std will be calculated across the entire image.
        self.slice_norm_method = None #If doing a slice-wise normalisation.
    ...
    def binary_im_to_model_dom(self, input_dom_im): #Similar logic to that borrowed from RadioActive SAMMed-2D inferer, some modifications were made to accomodate for SAM2's
        ...
        input_dom_im_backend = copy.deepcopy(input_dom_im)[0,:].numpy() #deepcopy just to be sure that we are not modifying the input image, and we are not using the metatensor
        del input_dom_im #Deleting the input image to free up memory, we will not need it anymore.
        ...
        for ax in self.app_params['image_axes']:
            ...
            # ax_slices_process = {} 
            slices_processed[ax] = {} #Insert directly and hope it takes up less memory.

            #This normalisation logic is standard as SAM2 does not provide their own preprocessing scripts aside from what is assumed for 
            ...
            # foreground is minimally shifted in intensity. And partially limited by the [0,255] expected.

            if self.glob_norm_bool:
                #lets add a clipping to prevent outliers from ruining it too.
                ...
                #Then we min-max normalisation on a global scale. Fairly standard.
                input_dom_im_backend = np.clip(input_dom_im_backend, lower_bound, upper_bound)
                input_dom_im_backend = np.round((input_dom_im_backend - lower_bound) / (upper_bound - lower_bound + 1e-6) * 255).astype(np.uint8)
            ...
            for slice_idx in range(input_dom_im_backend.shape[ax]):
                ...
                if not self.glob_norm_bool:      
                    if self.slice_norm_method == 'basic':             
                        ...
                        raise Exception(f'Normalisation method {self.norm_method} is not supported, only basic is supported for now.')
                ...
                slices_processed[ax][slice_idx] = slice
    ...
    def binary_extract_im_features(self, im_slices_model_dom:dict):
        ...
        for ax in self.app_params['image_axes']:
            ...
            # with torch.no_grad():
                img_slice = img_slice.to(device=self.infer_device, dtype=torch.float32)
                ...
                if self.model.directly_add_no_mem_embed:
                    vision_feats[-1] = vision_feats[-1] + self.model.no_mem_embed.detach() #Just being paranoid, detach not required
                    #in torch.no_grad context window.

                feats = [
                    feat.permute(1, 2, 0).view(1, -1, *feat_size)
                    for feat, feat_size in zip(vision_feats[::-1], self._bb_feat_sizes[::-1])
                ][::-1]
                #This is going to be quite taxing if we store all the axial slices of it, fingers crossed...............
                # ........
                #Well, that failed. HACK: This is absolutely diabolical, lets try store some on cpu and some on gpu?
                if slice_idx % self.cpu_gpu_burden_ratio:
                    #If modulo != 0 then stash that on cpu. otherwise, keep it on cuda device.
                    feats = [i.to(device='cpu') for i in feats] #yep very slow but the cost function has value infinity 
                #for code breaking!
                    #Might have hotfixed this, so we set this value to 1, so all are stored on cuda device. 
    ...
    def binary_slice_predict(
        ...
        if mask_input is not None and mask_input.shape[0] > 1: #In this case the mask has been pre-initialised with a set of bboxes to obtain instance-level masks.
            ...
            if concat_points[0].shape[0] != mask_input.shape[0]:
                ...
                mask_input = mask_input.repeat(concat_points[0].shape[0],1,1,1).to(device=self.infer_device)
            else:
                mask_input = mask_input.to(device=self.infer_device) #In this case, we just ensure that the mask is on the correct device.
        ...
        high_res_features = [
            feat_level[img_idx].unsqueeze(0).to(device=self.infer_device) #What does this actually do...? Well, in image-batch (not instance batch mode) mode the img idx is as described. So effectively extracting a given
        ...
        image_embeddings = image_features['image_embed'][img_idx].unsqueeze(0).to(self.infer_device)
        #I know we stored half on a cuda device already.... but realistically, this won't do anything if its on the infer device.. so its fine.
        ...
        #HACK: Please stop segfaulting!!
        high_res_features = [i.to(device='cpu') for i in high_res_features]
        ...
        if multimask_output:
            ...
            low_res_masks = low_res_masks[torch.arange(b_dim), max_index]
            assert low_res_masks.ndim == 3 # Shape: [B, H, W]
